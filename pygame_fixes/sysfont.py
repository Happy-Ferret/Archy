...
def initsysfonts_win32():
    ...
    for key in keys:
        ...
        for i in range(_winreg.QueryInfoKey(key)[1]):
            ...
            # Some font names have non-ascii characters in them.  The
            # easy (although not the nicest) way to deal with them is
            # to simply skip them, which is what the following try/except
            # clause does.
            try:
                font = str(font)
            except:
